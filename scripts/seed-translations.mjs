#!/usr/bin/env node
/**
 * Translation seeding pipeline (maintainer tool — adapted from DovesDataViewer's
 * scripts/seed-translations.mjs for this plugin's flat, single-namespace layout).
 *
 * Machine-translates the English source-of-truth bundle (panel/locales/en.json)
 * into every other shipped language, using an LLM and the motorsport glossary
 * (scripts/i18n-glossary.json). Re-runnable: it only translates keys that are
 * *new or changed* since the last run, and it never overwrites a string a human
 * has reviewed (see the _reviewed marker below).
 *
 * This is NOT part of the app or CI. It makes network calls and needs an API
 * key; the offline plugin never runs it — it only ships the JSON output.
 *
 * Usage:
...
const LOCALES_DIR = path.resolve(__dirname, "../panel/locales");
const SOURCE_LANG = "en";

// Keep in sync with the loaders in panel/i18n.ts (registerCoachLocale).
...
// --- Pure helpers (canonical copies live in lib/i18n/seedUtils.ts, which is
...
async function seedLanguage(language, glossary) {
  const sourceTree = await readJson(path.join(LOCALES_DIR, `${SOURCE_LANG}.json`));
  ...
  if (Object.keys(todo).length > 0) {
    ...
    console.log(`  ${language}: translating ${Object.keys(todo).length} key(s)…`);
  ...
  console.log(`  ${language}: wrote ${targetPath}`);
}
...
async function main() {
  ...
  console.log(`Seeding [${languages.join(", ")}] · model ${MODEL}`);
  for (const language of languages) {
    ...
    console.log(`\n${language}:`);
    await seedLanguage(language, glossary);
  }
